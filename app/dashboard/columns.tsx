...
export default function Columns({ isLoading }: { isLoading: boolean }) {
  ...
  [
  {
    ...
    cell: ({ row }) => isLoading ? <Skeleton animation="wave" height={30} /> : <div className="text-center ml-2 font-medium">#{row.getValue("student_number") || "-"}</div>,
    enableSorting: true,
    enableHiding: false,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (isNaN(date.getTime())) return <div className="text-center text-slate-400">—</div>;
